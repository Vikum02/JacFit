...
async function CreateUser() {
  return await __jacCallFunction("CreateUser", {});
}
...
async function DailyReport() {
  return await __jacCallFunction("DailyReport", {});
}
async function WeeklyTrend() {
  return await __jacCallFunction("WeeklyTrend", {});
}
async function ProgressCheck() {
  return await __jacCallFunction("ProgressCheck", {});
}
...
function app() {
  ...
  async function loadData() {
    setLoading(true);
    let r1 = await __jacSpawn("DailyReport", "", {"user_name": userName});
    setDailyData((_jac.builtin.bool(r1.reports) ? r1.reports[0] : null));
    let r2 = await __jacSpawn("WeeklyTrend", "", {"user_name": userName});
    setWeeklyData((_jac.builtin.bool(r2.reports) ? r2.reports[0] : null));
    let r3 = await __jacSpawn("ProgressCheck", "", {"user_name": userName});
  ...
  async function handleLogin() {
    ...
    let result = await jacLogin(authUser, authPass);
    if (result) {
  ...
  async function handleSignup() {
    ...
    let result = await jacSignup(authUser, authPass);
    if (result["success"]) {
      let loginResult = await jacLogin(authUser, authPass);
      if (loginResult) {
        setUserName(authUser);
        await __jacSpawn("CreateUser", "", {"name": authUser, "weight": 0.0, "height": 0.0, "goal_type": "maintain", "target_weight": 0.0, "daily_calorie_goal": 2000.0, "deadline": ""});
        setIsLoggedIn(true);
        loadData();
      }
    } else {
      setAuthError("Signup failed. Try a different username.");
    }
  }
  useEffect(() => {
    (async () => {
      ...
      if (isLoggedIn) {
        let storedName = (localStorage.getItem("jacfit_user") || "");
  ...
  let inputStyle = {"width": "100%", "padding": "10px 14px", "border": "1px solid #d1fae5", "borderRadius": "8px", "fontSize": "15px", "marginBottom": "12px"};
  if (!isLoggedIn) {
    return __jacJsx("div", {"style": {"minHeight": "100vh", "background": "#f0fdf4", "display": "flex", "alignItems": "center", "justifyContent": "center"}}, [__jacJsx("div", {"style": {"background": "#fff", "borderRadius": "16px", "padding": "40px", "width": "360px", "boxShadow": "0 4px 12px rgba(0,0,0,0.1)"}}, [__jacJsx("h1", {"style": {"color": "#166534", "textAlign": "center", "marginBottom": "8px", "fontSize": "1.8rem"}}, ["JacFit"]), __jacJsx("p", {"style": {"color": "#6b7280", "textAlign": "center", "marginBottom": "28px", "fontSize": "14px"}}, ["Diet & Fitness Tracker"]), __jacJsx("input", {"type": "text", "placeholder": "Username", "value": authUser, "onChange": e => {
      setAuthUser(e.target.value);
    }, "style": inputStyle}, []), __jacJsx("input", {"type": "password", "placeholder": "Password", "value": authPass, "onChange": e => {
      setAuthPass(e.target.value);
    }, "style": inputStyle}, []), ((authError && __jacJsx("p", {"style": {"color": "#dc2626", "fontSize": "13px", "marginBottom": "12px"}}, [authError])) || null), (((authMode === "login") && __jacJsx("div", {}, [__jacJsx("button", {"onClick": handleLogin, "style": {"width": "100%", "background": "#16a34a", "color": "#fff", "border": "none", "padding": "12px", "borderRadius": "8px", "fontSize": "16px", "fontWeight": "600", "marginBottom": "12px"}}, ["Login"]), __jacJsx("p", {"style": {"textAlign": "center", "color": "#6b7280", "fontSize": "14px"}}, ["No account?", __jacJsx("button", {"onClick": () => {
      setAuthMode("signup");
      setAuthError("");
    }, "style": {"background": "none", "border": "none", "color": "#16a34a", "fontWeight": "600", "cursor": "pointer", "marginLeft": "6px"}}, ["Sign Up"])])])) || __jacJsx("div", {}, [__jacJsx("button", {"onClick": handleSignup, "style": {"width": "100%", "background": "#16a34a", "color": "#fff", "border": "none", "padding": "12px", "borderRadius": "8px", "fontSize": "16px", "fontWeight": "600", "marginBottom": "12px"}}, ["Create Account"]), __jacJsx("p", {"style": {"textAlign": "center", "color": "#6b7280", "fontSize": "14px"}}, ["Have an account?", __jacJsx("button", {"onClick": () => {
      setAuthMode("login");
      setAuthError("");
    }, "style": {"background": "none", "border": "none", "color": "#16a34a", "fontWeight": "600", "cursor": "pointer", "marginLeft": "6px"}}, ["Login"])])]))])]);
  }
  return __jacJsx("div", {"style": {"minHeight": "100vh", "background": "#f0fdf4"}}, [__jacJsx("nav", {"style": {"background": "#16a34a", "padding": "16px 24px", "display": "flex", "gap": "16px", "alignItems": "center"}}, [__jacJsx("span", {"style": {"color": "#fff", "fontWeight": "700", "fontSize": "1.4rem", "marginRight": "24px"}}, ["JacFit"]), [__jacJsx("button", {"onClick": () => {
    setActiveTab("dashboard");
  }, "style": {"background": ((activeTab === "dashboard") ? "rgba(255,255,255,0.25)" : "transparent"), "color": "#fff", "border": "none", "padding": "8px 16px", "borderRadius": "8px", "fontWeight": "600", "fontSize": "14px"}}, ["Dashboard"]), __jacJsx("button", {"onClick": () => {
    setActiveTab("log_meal");
  }, "style": {"background": ((activeTab === "log_meal") ? "rgba(255,255,255,0.25)" : "transparent"), "color": "#fff", "border": "none", "padding": "8px 16px", "borderRadius": "8px", "fontWeight": "600", "fontSize": "14px"}}, ["Log Meal"]), __jacJsx("button", {"onClick": () => {
    setActiveTab("log_exercise");
  }, "style": {"background": ((activeTab === "log_exercise") ? "rgba(255,255,255,0.25)" : "transparent"), "color": "#fff", "border": "none", "padding": "8px 16px", "borderRadius": "8px", "fontWeight": "600", "fontSize": "14px"}}, ["Log Exercise"]), __jacJsx("button", {"onClick": () => {
    setActiveTab("weekly");
